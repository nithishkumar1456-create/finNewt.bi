...
exports.createBudget = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const budget = await budgetService.createBudget(req.user.id, req.body);
    res.status(201).json({ success: true, data: { budget } });
});
exports.updateBudget = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const budget = await budgetService.updateBudget(req.user.id, req.params.id, req.body);
    res.status(200).json({ success: true, data: { budget } });
});
exports.deleteBudget = (0, catchAsync_1.catchAsync)(async (req, res) => {
    await budgetService.deleteBudget(req.user.id, req.params.id);
    res.status(200).json({ success: true, message: 'Budget deleted successfully' });
});
exports.getBudgets = (0, catchAsync_1.catchAsync)(async (req, res) => {
