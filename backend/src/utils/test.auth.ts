...
const runE2E = async () => {
  ...
  try {
    ...
    const dbUserBefore = await prisma.user.findUnique({ where: { email: TEST_EMAIL } });
    if (dbUserBefore) throw new Error('User was created before verification! BUG!');
    ...
    const dbUserAfter = await prisma.user.findUnique({ where: { email: TEST_EMAIL } });
    if (!dbUserAfter) throw new Error('User was NOT created after verification! BUG!');
    ...
    await prisma.user.delete({ where: { id: dbUserAfter.id } });
