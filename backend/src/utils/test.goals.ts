...
const runE2E = async () => {
  ...
  try {
    ...
    if (Number(addFundsRes.data.goal.currentAmount) !== 3000) throw new Error('Add funds math failed!');
    ...
    if (editRes.data.goal.status !== 'completed') throw new Error('Goal status completion logic failed!');
