...
export const getSummary = async (userId: string, startDate?: string, endDate?: string) => {
  ...
  transactions.forEach(t => {
    const amount = typeof t.amount === 'object' ? (t.amount as any).toNumber() : Number(t.amount);
...
export const getTrends = async (userId: string, type: 'weekly' | 'monthly') => {
  const transactions = await prisma.transaction.findMany({
    where: { userId },
    orderBy: { date: 'asc' },
  });
  ...
  transactions.forEach(t => {
    ...
    const amount = typeof t.amount === 'object' ? (t.amount as any).toNumber() : Number(t.amount);
    if (t.type === 'INCOME') groupedData[key].income += amount;
    if (t.type === 'EXPENSE') groupedData[key].expense += amount;
  });

  // Sort by key to ensure chronological order in charts
  const sortedData: Record<string, { income: number, expense: number }> = {};
...
export const getCategories = async (userId: string, startDate?: string, endDate?: string) => {
  const dateFilter: any = {};
  if (startDate) {
